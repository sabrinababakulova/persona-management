...
- **Multi-tenancy**: every domain entity (`candidates`, `vacancies`) is scoped by `companyId`, auto-assigned from the creating user's company. Queries filter by the current user's company — users only see their own company's data. Registration either creates a company (the creator becomes its single `admin`) or joins one via an invite link (`member`); only the admin can edit the company profile or manage invites. See `README.md` for the full role/sign-up flow.
