...
- **Multi-tenancy**: every domain entity (`candidates`, `vacancies`) is scoped by `companyId`, auto-assigned from the creating user's company. Queries filter by the current user's company — users only see their own company's data.
