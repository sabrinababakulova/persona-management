...
```
/
├── README.md                 # Canonical bilingual product and operations documentation
...
### Company roles
_Full walk-through of roles, sign-up, and the invite flow: `README.md`._
...
- Documentation: `README.md`. Schema helpers: `src/server/api/routers/vacancies/schemas.ts` (`PersonHunterPublicationMeta`).
...
### Key files
`hh/negotiations.ts` (rich negotiation fetch), `hh/discover-candidates.ts`, `hh/enrich-worker.ts`, `hh/sync-statuses.ts`, `hh-company-account.ts`, `src/server/api/routers/candidates/sync.ts`, and `README.md`.
